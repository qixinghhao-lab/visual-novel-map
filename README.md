...
## 故事线：把灵感放到节点上，把正文交给线段去串

故事线是这个项目最重要的页面。它不是单纯画一条时间轴，而是把小说结构拆成三类东西：章节节点、逻辑点、节点之间的故事线。

章节节点负责标记一章和下一章之间的接力点。逻辑点负责记录你突然想到的关键剧情、转折、伏笔或冲突。节点之间的故事线负责回答另一个问题：两个点都想好了，中间到底怎么自然写过去？

这个项目的核心使用方式就是围绕这三件事展开的：先把灵光一现的剧情点丢到节点里，再让节点之间的故事线承担“串起来”的任务。如果生成出来的衔接不满意，就直接点那一段线，把你的新想法写进去，让它按这个方向重新组织。
...
点击章节节点，就会弹出节点浮窗。这个浮窗不是让你写完整章节，而是让你先把“这一章承接什么、下一章要从哪里打开”记下来。
...
逻辑点比章节节点更轻。它不一定是一章，也不一定是一个完整场景，更像是你在写作时突然想到的一个“必须发生”的剧情钉子。

比如：某个角色必须在这里发现线索，某个误会必须在这里扩大，某个伏笔必须在这里露出一点边，某个情绪必须在这里降下去。它们太小，不适合单独开一章；但它们又太重要，不能只写在备忘录里。
...
如果你觉得这段衔接不满意，也不需要回到一大段正文里慢慢找。直接点击节点之间那一段故事线，在弹出的对话框里写你的想法：比如“这里不要太快揭露”“增加一段对话”“让情绪更压抑一点”“把冲突提前”。然后让 AI 按这个方向重新修改这一段。
